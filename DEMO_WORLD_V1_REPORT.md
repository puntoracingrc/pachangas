...
| DW-033 | PRODUCT_BUG | open | El smoke anonimo posterior al merge revelo que `/` seguia mostrando la demo heredada completa cuando no habia sesion ni equipo, aunque `?demo=1` ya redirigia a `/demo`. Quedaban dos demos publicas competidoras y el acceso principal no enseñaba un CTA al Mundo Demo V1. | Pendiente de corregir en follow-up de release; no se oculta ni se reclasifica como una limitacion aceptada. | Pendiente: raiz anonima debe mostrar entrada de producto compacta, abrir `/demo` mediante CTA y no renderizar plantilla, partido ni historial ficticios heredados. |
