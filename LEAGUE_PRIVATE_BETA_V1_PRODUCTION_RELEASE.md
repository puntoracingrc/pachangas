...
| LPB-018 | SIMULATION_BUG | The first LPB-017 production re-smoke completed its product actions but the temporary result projection used an ambiguous `competition_id` reference, so PostgreSQL aborted the entire diagnostic transaction before returning evidence. | detected + rollback_verified | Immediate readback confirms transaction rollback with 0 active bundles, 0 draft wizards, 0 active beta competitions and 0 rows for the attempted QA slug. The corrected projection and full smoke remain pending. |
| LPB-019 | SIMULATION_BUG | The corrected result projection assumed `pachanga_canonical_matches` stored `competition_id` directly instead of resolving the canonical Competition binding, so its final evidence query failed after the product actions. | detected + rollback_verified | PostgreSQL again rolled back the complete diagnostic transaction; immediate readback confirms 0 active bundles, wizards or beta competitions and 0 rows for the QA slug. The binding-aware projection and full smoke remain pending. |
