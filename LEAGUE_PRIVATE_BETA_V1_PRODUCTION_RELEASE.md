...
| LPB-017 | PRODUCT_BUG | The first production smoke finalized the wizard with the Competition in `draft` but the Edition already in `registration_open`, contrary to the release contract that requires both aggregates to remain drafts until a later canonical registration command. | detected + cleanup_verified | The smoke Competition was cancelled before fixtures and its exact beta bundle was revoked. A forward-only correction and regression are required before closing the release. |
