  ...
  return (
    <main className={styles.shell}>
      ...
      <div className={styles.content}>
        ...
        {tab === "conducta" ? (
          <section className={styles.twoColumns}>
            <div className={styles.panel}><header><div><span>Conducta V1.1 · sombra</span><h2>Colas recomendadas</h2></div><b>{data.conductTriageV11.humanReviewCases}/{data.conductTriageV11.cases}</b></header><CountRows values={data.conductTriageV11.queues} /><p className={styles.muted}>Recall grave {(data.conductTriageV11.seriousCaseRecall * 100).toFixed(1)}% · precisión {(data.conductTriageV11.seriousCasePrecision * 100).toFixed(1)}% · {data.conductTriageV11.missedSeriousCases} graves omitidos. Sin sanciones automáticas.</p></div>
            <div className={styles.panel}><header><div><span>Capacidad</span><h2>Backlog simulado</h2></div><b>{data.conductTriageV11.humanReviewCases}</b></header><ul className={styles.decisionList}><li>5 casos/día: {data.conductTriageV11.backlog.casesPerDay5.backlog} pendientes, espera media {data.conductTriageV11.backlog.casesPerDay5.meanWaitDays} días.</li><li>10 casos/día: {data.conductTriageV11.backlog.casesPerDay10.backlog} pendientes, espera media {data.conductTriageV11.backlog.casesPerDay10.meanWaitDays} días.</li><li>10.000 jugadores: {data.conductTriageV11.projectedHumanCases.players10000} revisiones orientativas.</li><li>50.000 jugadores: {data.conductTriageV11.projectedHumanCases.players50000} revisiones orientativas.</li></ul></div>
            <div className={styles.panel}><header><div><span>No-show · comparativa</span><h2>Umbrales sin activar</h2></div><b>{data.conductTriageV11.noShowThresholds.confirmedNoShowEvents}</b></header><ul className={styles.decisionList}>{data.conductTriageV11.noShowThresholds.variants.map((variant) => <li key={`${variant.count}-${variant.days}`}>{variant.count} en {variant.days} días: {variant.candidates} candidatos.</li>)}</ul><p className={styles.muted}>{data.conductTriageV11.noShowThresholds.distributionAssessment === "stress_distribution_not_product_calibration" ? "Distribución de estrés: no apta para recalibrar producto." : "Distribución plausible, todavía sin validación real."}</p></div>
            <div className={styles.panel}><header><div><span>Protecciones</span><h2>Falsas escaladas</h2></div><b>{(data.conductTriageV11.falseEscalationRate * 100).toFixed(1)}%</b></header><ul className={styles.decisionList}><li>Campañas falsas elevadas: {data.conductTriageV11.falseCampaignEscalation}.</li><li>Reporte aislado con historial limpio elevado: {data.conductTriageV11.singleCleanReportEscalation}.</li><li>Rating V2, Season Score, TOPS, logros y recompensas: intactos.</li></ul></div>
            <div className={styles.panel}><header><div><span>Asistencia V1</span><h2>Cancelación no es no-show</h2></div><b>{data.conductV1.attendance.finalConfirmedNoShows}</b></header><CountRows values={data.conductV1.attendance.byFinalOutcome} /><p className={styles.muted}>{data.conductV1.attendance.candidateNoShows} candidatos revisados; {data.conductV1.attendance.disputed} disputas y {data.conductV1.attendance.corrections} correcciones. {data.conductV1.attendance.normalCancellations} cancelaciones normales produjeron {data.conductV1.attendance.falsePositiveNoShowsFromNormalCancellations} falsos no-show.</p></div>
            <div className={styles.panel}><header><div><span>Conducta V1</span><h2>Casos agrupados</h2></div><b>{data.conduct.cases}</b></header><CountRows values={data.conduct.byKind} /><p className={styles.muted}>{data.conduct.correlatedReports} clics correlacionados se agruparon; {data.conduct.independentSources} fuentes independientes. {data.conduct.restrictionRecommendations} recomendaciones, {data.conduct.restrictionsApplied} restricciones automáticas. Rating V2 permanece intacto.</p></div>
            ...
            <div className={styles.panel}><header><div><span>Carga humana</span><h2>Moderación orientativa</h2></div><b>{data.conduct.humanCases}</b></header><ul className={styles.decisionList}><li>1.000 jugadores: {data.conductV1.moderationLoad.projectedCases.players1000} casos/temporada.</li><li>5.000 jugadores: {data.conductV1.moderationLoad.projectedCases.players5000} casos/temporada.</li><li>10.000 jugadores: {data.conductV1.moderationLoad.projectedCases.players10000} casos/temporada.</li><li>La elegibilidad de trofeos durante una restricción social sigue pendiente de decisión.</li></ul></div>
          </section>
        ) : null}
