...
export default async function PlatformFlagsPage() {
  ...
  return <><PageHeader title="Feature flags" subtitle="Producto, laboratorio y funciones apagadas se muestran como estados distintos. Los secrets no aparecen aquí." />
    <Panel><DataTable label="Feature flags"><thead><tr><th>Función</th><th>Estado</th><th>Preparación</th><th>Valor</th><th>Fuente</th><th>Revisión</th><th>Acción</th></tr></thead><tbody>{flags.map((flag) => <tr key={String(flag.key)}><td><strong>{String(flag.label ?? flag.key)}</strong><small>{String(flag.key)}</small></td><td><StatusBadge tone={flag.state === "PRODUCT" ? "good" : flag.state === "LAB" ? "info" : "muted"}>{String(flag.state)}</StatusBadge><small>{String(flag.classification ?? "-")}</small></td><td><strong>{String(flag.readiness ?? "-")}</strong><small>{String(flag.readinessReason ?? "Sin diagnóstico")}</small>{flag.dependency ? <small>Dependencia: {String(flag.dependency)}</small> : null}</td><td>{flag.enabled ? "Activo" : "Inactivo"}{flag.effectiveFrom ? <small>Desde {new Date(String(flag.effectiveFrom)).toLocaleString("es-ES")}</small> : null}</td><td><code className={styles.identifier}>{String(flag.source ?? "-")}</code></td><td>{String(flag.revision ?? 0)}</td><td>{canWrite && flag.mutable ? <FlagChangeControl enabled={Boolean(flag.enabled)} flagKey={String(flag.key)} revision={Number(flag.revision) || 0} /> : <span className={styles.muted}>Solo lectura</span>}</td></tr>)}</tbody></DataTable></Panel>
  </>;
}
