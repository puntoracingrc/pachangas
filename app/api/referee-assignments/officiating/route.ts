...
export async function POST(request: Request) {
  try {
    ...
    const payload = refereeOfficiatingPayload(envelope.action, refereeRecord(envelope.body.payload));
    const result = await client.rpc("command_pachanga_referee_officiating_v1", {
      client_metadata: refereeAssignmentMetadata(request, "referee_match_officiating"),
      command_action: envelope.action,
      command_payload: payload,
      expected_revision: envelope.expectedRevision,
      operation_id: envelope.operationId,
      target_assignment_id: envelope.aggregateId,
    });
