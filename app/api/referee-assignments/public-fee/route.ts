...
export async function POST(request: Request) {
  try {
    ...
    const payload = refereePublicFeePayload(envelope.action, refereeRecord(envelope.body.payload));
    const result = await client.rpc("command_pachanga_referee_public_fee_v1", {
      aggregate_id: envelope.aggregateId,
      client_metadata: refereeAssignmentMetadata(request, "referee_public_fee"),
