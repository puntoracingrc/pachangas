...
export async function GET(request: Request, { params }: { params: Promise<{ competitionId: string }> }) {
  try {
    ...
    const result = await client.rpc("get_pachanga_league_postponement_desk_v1", {
      page_offset: Math.max(0, Number.parseInt(url.searchParams.get("offset") ?? "0", 10) || 0),
      page_size: Math.min(200, Math.max(1, Number.parseInt(url.searchParams.get("limit") ?? "50", 10) || 50)),
