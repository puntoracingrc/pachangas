...
export async function POST(request: Request) {
  try {
    ...
    const action = parseTournamentAction(body.action);
    ...
    const result = await client.rpc("command_pachanga_tournament_draw_v1", {
      aggregate_id: aggregateId,
      client_metadata: tournamentClientMetadata(request, "tournament_draw"),
      command_action: action,
      command_payload: tournamentCommandPayload(action, tournamentRecord(body.payload)),
