import {
  parseTournamentGroupStageAction,
  requireTournamentOrigin,
  tournamentClientMetadata,
  tournamentError,
  tournamentGroupStageCommandPayload,
  tournamentJson,
  tournamentRecord,
  tournamentSession,
...
export async function POST(request: Request) {
  try {
    ...
    const action = parseTournamentGroupStageAction(body.action);
    ...
    const result = await client.rpc("command_pachanga_tournament_group_stage_v1", {
      aggregate_id: aggregateId,
      client_metadata: tournamentClientMetadata(request, "tournament_group_stage"),
      command_action: action,
      command_payload: tournamentGroupStageCommandPayload(action, tournamentRecord(body.payload)),
