...
export function DemoSocialFirstTimeJourney({ onClose, onNavigate }: {
  ...
  return (
    <div className={styles.backdrop} role="presentation">
      <section className={styles.journey} role="dialog" aria-modal="true" aria-labelledby="demo-social-title" data-demo-social-first-time="v3e" onKeyDown={handleDialogKeyDown} ref={dialogRef}>
        <header>
        ...
        {state.stage === "availability" ? <div className={styles.body}>
          ...
          <Actions secondary="Volver" onSecondary={() => patch({ stage: "profile" })} primary="Continuar" onPrimary={() => patch({ stage: "start", zone: state.zone.trim() || "Barcelona" })} />
        </div> : null}

        {state.stage === "start" ? <div className={styles.body}>
          <Step number="3" title="Cómo quieres empezar" copy="Elige una dirección. Todo sigue siendo local y reversible." />
        ...
        {state.stage === "join" ? <div className={styles.body}>
          ...
          <Actions secondary="Volver" onSecondary={() => patch({ stage: "start" })} primary="Buscar equipo" onPrimary={() => {
            ...
            patch({ stage: "join-confirm" });
            setMessage("Equipo ficticio localizado. Falta tu confirmación.");
          }} />
        ...
        {state.stage === "join-confirm" ? <div className={styles.body}>
          <Step number="2/2" title="Confirma el equipo" copy="Revisa la identidad antes de unirte. Nada se acepta automáticamente." />
          <article className={styles.teamPreview}><i aria-hidden="true">CR</i><div><span>Equipo encontrado</span><strong>Cobalto Raval</strong><p>Fútbol 7 · Barcelona · 14 miembros · rol jugador</p></div></article>
          <Actions secondary="Volver" onSecondary={() => patch({ stage: "join" })} primary="Confirmar unión" onPrimary={() => { patch({ activeTeam: "Cobalto Raval", createdLocally: false, stage: "ready" }); setMessage("Unión confirmada solo en esta sesión demo."); }} />
        </div> : null}
        ...
        {state.stage === "ready" ? <div className={styles.body}>
          ...
          <p className={styles.message}>{state.offline ? "Modo offline: lecturas disponibles, escrituras bloqueadas." : message}</p>
        </div> : null}
