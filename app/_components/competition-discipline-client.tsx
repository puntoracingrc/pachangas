...
function PlayerStates({ competitionId, data }: { competitionId: string; data: CompetitionDisciplineJson }) {
  return <section className={styles.band}><SectionHeader eyebrow="Elegibilidad" title="Estado de jugadores" /><div className={styles.playerGrid}>{disciplineArray(data.playerStates).map((item) => {
    const unavailable = ["active", "provisional"].includes(disciplineText(item.status));
    return <Link href={`/competiciones/${competitionId}/jugadores/${disciplineText(item.playerProfileId)}/disciplina`} key={disciplineText(item.id)}><span><strong>{playerName(item)}</strong><Status value={item.status} /></span><small>{unavailable ? "No disponible por sanción · " : ""}{disciplineNumber(item.remainingUnits)} {disciplineText(item.unitType).toLowerCase()} pendientes</small><div>{Object.entries(disciplineRecord(item.cards)).map(([card, value]) => <em key={card}><CardMark code={card} />{disciplineNumber(disciplineRecord(value).events)}</em>)}</div></Link>;
