...
Las incidencias `SW-0059` a `SW-0067` quedaron registradas antes de corregirse y cerradas con regresión. Cubren la ambigüedad de unidades, la contaminación por controles automáticos, la clasificación de cohorts de confidence, el drift de configuración V3, el etiquetado de percentiles, la identidad duplicada de filas React, la precondición del runner de concurrencia, dos símbolos muertos detectados por lint focalizado y el arranque controlado del dashboard local. La configuración del adaptador vuelve a coincidir exactamente con V3; Rating V2 permanece intacto.
