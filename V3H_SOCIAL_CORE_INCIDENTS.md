...
| V3H-032 | ACCESSIBILITY | fixed + regression_verified | Las tarjetas, historial y detalle de Retos conservaban superficies oscuras fijas en tema claro, pero heredaban texto oscuro y perdían contraste en landscape | Tokens de superficie light/dark, V3H 7/7, batería 825/825 y contact sheets desktop/portrait/landscape regeneradas |
| V3H-033 | ENVIRONMENT_ISSUE | fixed + regression_verified | El primer deployment productivo integrado con Git restauró un chunk CSS global obsoleto aunque el árbol de `main` era correcto; la matriz estructural no falló, pero la inspección visual manual detectó la Landing parcialmente sin estilo | Redespliegue productivo forzado sin caché `dpl_F8paNkcfNM5jdK24bmCwRmBehsYR`; CTA calculada como `display:flex`, 48 px y verde; matriz pública 88/88 sin overflow, imágenes rotas, errores, warnings ni targets pequeños |
...
V3H-033 se aisló y conservó con evidencia antes del redespliegue de emergencia. Su entrada persistente se añadió inmediatamente después de recuperar producción; no se ocultó ni se reinterpretó como defecto de fuente.
