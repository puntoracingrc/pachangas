...
test("the base library stays stable while Premium RC selects 18 V1 candidates", () => {
  assert.equal(TEAM_SHIELD_BASE_CATALOG.length, 28);
  assert.equal(TEAM_SHIELD_COSMETIC_PROTOTYPES.length, 32);
  assert.equal(TEAM_SHIELD_COSMETIC_V1_CANDIDATES.length, 18);
  assert.equal(new Set(TEAM_SHIELD_BASE_CATALOG.map(({ key }) => key)).size, 28);
  assert.equal(new Set(TEAM_SHIELD_COSMETIC_V1_CANDIDATES.map(({ key }) => key)).size, 18);
