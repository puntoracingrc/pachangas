...
const upgradeDatabaseName = `pachangas_wave9a_upgrade_${suffix}`;
const freshDatabaseName = `pachangas_wave9a_fresh_${suffix}`;
const infrastructureDump = resolve(tmpdir(), `pachangas-wave9a-infra-${suffix}.sql`);
let cleanupFailure;

if (!["127.0.0.1", "localhost", "::1", "[::1]"].includes(new URL(adminUrl).hostname)) {
  throw new Error("VENUE_OPERATIONS_DB_TEST_LOCAL_DATABASE_REQUIRED");
}
...
assert.equal(migrations.filter((name) => !waveMigrations.includes(name)).length, 212);
...
function dropDatabase(name) {
  ...
  if (exists === "1") {
    ...
    admin(`select pg_terminate_backend(pid) from pg_stat_activity where datname=${quote(name)} and pid<>pg_backend_pid()`, `terminate ${name} clients`);
...
try {
  ...
  ], "prepare exact 212-migration Wave 9A base");
  applyTo(
    upgradeDatabaseName,
  ...
  ], "fresh bootstrap to 220");

  const upgradedSchema = normalizedSchema(upgradeDatabaseName);
  const freshSchema = normalizedSchema(freshDatabaseName);
  assert.equal(freshSchema, upgradedSchema, "fresh and 212-to-220 schemas must be identical");
  schemaHash = createHash("sha256").update(upgradedSchema).digest("hex");
  assert.equal(
    query(upgradeDatabaseName, ["-Atq", "-c", `select count(*) from private.pachanga_venue_settings_v1 where singleton and not venue_foundation_enabled and not venue_management_enabled and not venue_payments_enabled`], "verify Venue flags born OFF"),
    "1",
  );
  const functionalOutput = query(upgradeDatabaseName, [
    "-q", "--single-transaction",
    "-f", resolve(root, "tests/venue-operations-v1-fixture.sql"),
    "-f", resolve(root, "tests/venue-operations-v1-db.sql"),
  ], "run Wave 9A canonical DB/RLS/DST lifecycle suite");
  assert.match(functionalOutput, /VENUE_OPERATIONS_V1_DB_PASS/);
} finally {
  try {
    cleanup();
  } catch (error) {
