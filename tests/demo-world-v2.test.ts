...
} from "../scripts/demo-world/demo-world-verification-projection";
...
test("Demo World V3 is deterministic and the committed snapshot matches its hash", async () => {
  ...
  assert.equal(committed.manifest.hash, "7d1de10a66147c7da53a062db4d96f59fa4b592b0dce6ef9c749044314dcd53c");
...
test("the committed authority proof comes from deterministic PostgreSQL operations", async () => {
  ...
  assert.equal(proof.authorityHash, "441370331d462172adf6efa347b16587ad36ef73558f376157d08441fd39c2f1");
  ...
  assert.equal(proof.migrationCount, 212);
...
test("V3.1 Team operational safety comes from isolated canonical RPCs", async () => {
  ...
  const canonicalAuthority = assertDemoWorldV2AuthorityProof(loadDemoWorldV2AuthorityProof());
  ...
  assert.equal(
    proof.competitionContinuity.sourceAuthorityHash,
    demoWorldVerificationProjectionHash(canonicalAuthority),
  );
  assert.notEqual(proof.competitionContinuity.sourceAuthorityHash, canonicalAuthority.authorityHash);
...
test("the protagonist League has the complete canonical R1-R5 graph including R3 assignments", async () => {
  ...
  assert.equal(league.provenance.migrations, 212);
...
test("the historical V2.1 through V3.0 snapshots remain immutable beside V3.1", async () => {
  ...
  const v27Files = [...v24Files, "public-competitions.json"];
  await Promise.all(v27Files.map((name) => readFile(path.join(historicalV27Root, name), "utf8")));
  const v27Manifest = JSON.parse(await readFile(path.join(historicalV27Root, "manifest.json"), "utf8")) as Record<string, unknown>;
...
test("V2.9 organizer billing comes from canonical TEST lifecycle operations without live commerce", async () => {
  ...
  assert.equal(billing.catalog.liveCheckoutEnabled, false);
  assert.equal(billing.catalog.status, "CATALOG_AVAILABLE");
  assert.deepEqual(billing.catalog.plans.map(({ planCode }) => planCode), [
    "CLUB_PARTNER",
    "CLUB_ORGANIZER",
    "TEAM_ORGANIZER_PRO",
  ]);
  ...
  assert.match(serialized, /AWAITING_PRICE_APPROVAL/);
