...
const databaseName = `pachangas_wave8a_db_${suffix}`;
const freshDatabaseName = `pachangas_wave8a_fresh_${suffix}`;
const infrastructureDump = resolve(tmpdir(), `pachangas-wave8a-db-${suffix}.sql`);
let cleanupFailure;
...
assert.equal(migrations.filter((name) => !waveMigrations.includes(name)).length, 197);
...
try {
  ...
  admin(`create database ${databaseName} template template0`, "create Wave 8A SQL database");
  query(["-q", targetUrl(), "-f", infrastructureDump], "restore Wave 8A SQL infrastructure");
  query(["-Atq", targetUrl(), "-c", "create publication supabase_realtime"], "create Wave 8A SQL Realtime publication");
  applyTo(databaseName, [
    resolve(root, manifest.baselinePath),
    ...preWaveMigrations.map((name) => resolve(root, "supabase/migrations", name)),
  ], "prepare exact 197-migration Wave 8A base");
  assert.equal(query(["-Atq", targetUrl(), "-c", "select to_regclass('private.pachanga_organizer_access_applications_v1') is null"], "verify pre-Wave 8A base"), "t");
  applyTo(databaseName, waveMigrations.map((name) => resolve(root, "supabase/migrations", name)), "upgrade 197 to 204");

  admin(`create database ${freshDatabaseName} template template0`, "create Wave 8A fresh database");
  query(["-q", targetUrl(freshDatabaseName), "-f", infrastructureDump], "restore Wave 8A fresh infrastructure");
  query(["-Atq", targetUrl(freshDatabaseName), "-c", "create publication supabase_realtime"], "create Wave 8A fresh Realtime publication");
  ...
  ], "fresh bootstrap to 204");
  const upgradedSchema = normalizedSchema(databaseName);
  const freshSchema = normalizedSchema(freshDatabaseName);
  assert.equal(freshSchema, upgradedSchema, "fresh and 197-to-204 schemas must be identical");
  schemaHash = createHash("sha256").update(upgradedSchema).digest("hex");
  query(["-q", targetUrl(), "-f", resolve(root, "tests/organizer-access-onboarding-v1-fixture.sql")], "load Wave 8A SQL fixture");
  output = query([
    "-Atq", targetUrl(), "-c", "begin",
    "-f", resolve(root, "tests/organizer-access-onboarding-v1-db.sql"),
    "-c", "rollback",
  ], "Wave 8A SQL, RLS, idempotency and onboarding suite");
  assert.match(output, /ORGANIZER_ACCESS_ONBOARDING_V1_DB_PASS/);
} finally {
  try {
    cleanup();
  } catch (error) {
...
process.stdout.write(`${JSON.stringify({
  ...
  schemaHash,
  sqlRlsIdempotencyOnboarding: "PASS",
  cleanup: "PASS",
})}\n`);
