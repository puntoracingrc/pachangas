...
test("keeps the project wired to the Pachangas app", async () => {
  ...
  assert.match(globalsCss, /input\[type="date"\]::-webkit-calendar-picker-indicator\s*\{[\s\S]*opacity:\s*0\.85/);
  assert.doesNotMatch(globalsCss, /input\[type="date"\]::-webkit-calendar-picker-indicator\s*\{[\s\S]{0,80}opacity:\s*0\s*;/);
  ...
  assert.match(page, /function requestNativeDatePicker/);
  assert.match(page, /showPicker\?\.\(\)/);
  assert.match(page, /onPointerDown=\{\(event\) => \{\s*if \(event\.button === 0\) requestNativeDatePicker\(event\.currentTarget\);/);
