...
test("builds Pachangas IQ HTML", async () => {
  ...
  assert.match(html, /Organiza tu pachanga\./);
  assert.match(html, /Invita a tus amigos, crea los equipos y juega\./);
  assert.match(html, /data-product-entry="no-team"/);
  assert.match(html, /Probar Demo/);
  assert.match(html, /href="\/demo\?review=1"/);
  assert.match(html, /Entrar/);
  assert.match(html, /Buscar una pachanga/);
  assert.match(html, /href="\/mercado"/);
  assert.match(html, /Crea el partido\./);
  assert.match(html, /Reúne a los jugadores\./);
  assert.match(html, /Juega y guarda el resultado\./);
  assert.match(html, /SIMULACIÓN/);
  assert.match(html, /(?:\/brand\/pachangas-logo-hero\.png|%2Fbrand%2Fpachangas-logo-hero\.png)/);
  ...
  assert.doesNotMatch(html, /Jugadores del próximo partido/);
  assert.doesNotMatch(html, /Finalizar partido/);
  assert.doesNotMatch(html, /Tiempo previsto/);
  assert.doesNotMatch(html, /Registro/);
  assert.doesNotMatch(html, /Entra para crear tu equipo/);
  assert.doesNotMatch(html, /El grupo del partido, pero con memoria\./);
  assert.doesNotMatch(html, /Mundo Demo social/);
  assert.doesNotMatch(html, /Your site is taking shape|Building your site|react-loading-skeleton/i);
});

test("keeps the player card laboratory out of search indexes", async () => {
  const html = await readFile(
    new URL("../.next/server/app/laboratorio-ficha-jugador.html", import.meta.url),
    "utf8",
  );
...
test("builds the transfer market as a separated page", async () => {
  ...
  assert.match(source, /return "partidos"/);
  assert.match(source, /\{ id: "partidos", label: "Partidos"/);
  assert.match(source, /\{ id: "jugadores", label: "Jugadores"/);
  assert.match(source, /\{ id: "equipos", label: "Equipos"/);
...
test("keeps portrait mobile views compact and readable", async () => {
  ...
  const portraitStart = css.indexOf("/* Portrait game mode keeps the compact manager language");
  const portraitCss = css.slice(portraitStart);
...
test("keeps the project wired to the Pachangas app", async () => {
  const [
    ...
    readFile(new URL("../supabase/migrations/20260803053543_rating_v2_privacy_reads.sql", import.meta.url), "utf8"),
    readFile(new URL("../supabase/migrations/20260803053625_rating_v2_authoritative_mutations.sql", import.meta.url), "utf8"),
    readFile(new URL("../supabase/migrations/20260803053632_rating_v2_profile_authority.sql", import.meta.url), "utf8"),
    readFile(new URL("../supabase/migrations/20260803053635_rating_v2_match_authority.sql", import.meta.url), "utf8"),
    readFile(new URL("../supabase/migrations/20260803053825_rating_v2_assessment_authority.sql", import.meta.url), "utf8"),
    readFile(new URL("../supabase/migrations/20260803062830_rating_v2_legacy_write_closure.sql", import.meta.url), "utf8"),
  ...
  assert.match(page, /social-entry-footer/);
  ...
  assert.doesNotMatch(page, /<OfficialSecondaryActions/);
  ...
  assert.match(page, /type MatchManagerPane = "proximo" \| "campo" \| "alineacion" \| "resultado" \| "admin"/);
  ...
  assert.match(page, /const showReserveRosterColumn = matchFinalized[\s\S]*activeMatch\.reservesAttend && reservePlayers\.length > 0/);
  assert.match(page, /const showWaitingRosterColumn = !matchFinalized && waitingPlayers\.length > 0/);
  assert.match(page, /const showStatusRosterColumn = !matchFinalized/);
  assert.match(page, /historical-match-admin-hub/);
  assert.match(page, /Administración del partido histórico/);
  assert.match(page, /Eliminar histórico/);
  ...
  assert.match(page.slice(nextMatchStatsStart - 80, nextMatchStatsStart), /!matchFinalized \? \(/);
  ...
  assert.match(page, /contextOptions=\{shellContextOptions\.length > 0 \? shellContextOptions : undefined\}/);
  ...
  assert.match(page, /\? \["proximo", "resultado", "campo"\]/);
  assert.match(page, /: \["proximo", "campo", "alineacion"\]/);
  ...
  assert.match(globalsCss, /\.historical-match-admin-actions[\s\S]*grid-template-columns:\s*minmax\(0,\s*1fr\)/);
  ...
  assert.match(globalsCss, /data-match-manager-pane="proximo"[\s\S]*\.payer-note,[\s\S]*data-match-manager-pane="proximo"[\s\S]*\.subscription-contribution-note,[\s\S]*display:\s*none !important/);
  ...
  assert.match(page, /Organiza tu pachanga\./);
  assert.match(page, /Probar Demo/);
  assert.match(page, /href="\/demo\?review=1"/);
  assert.match(page, /La Demo es una <b>SIMULACIÓN<\/b>/);
  ...
  assert.doesNotMatch(page, /onPlayerClick=\{setPitchPreviewPlayerId\}/);
  assert.match(page, /className=\{`pitch-player-card/);
  assert.match(page, /\$\{playerDisplayName\(player\)\} en el campo/);
  assert.match(page, /SearchLogo/);
  assert.match(page, /pitchZoomOpen/);
  assert.doesNotMatch(page, /pitchPreviewPlayer/);
  assert.doesNotMatch(page, /PitchPlayerPreview/);
  ...
  assert.doesNotMatch(page, /suppressPitchPreviewUntilRef/);
  ...
  assert.doesNotMatch(globalsCss, /\.pitch-player-preview-card/);
  ...
  assert.doesNotMatch(page, /setPitchPreviewPlayerId/);
  ...
  assert.match(page, /\{ label: "Próximos", value: openMatches\.length \}/);
  assert.match(page, /\{ label: "Plantilla", value: activeRosterCount \}/);
  ...
  assert.match(page, /step=\{600\}/);
  assert.match(page, /function requestNativeInputPicker/);
  assert.match(page, /showPicker\?\.\(\)/);
  assert.match(page, /onPointerDown=\{\(event\) => \{\s*if \(event\.button === 0\) requestNativeInputPicker\(event\.currentTarget\);/);
  assert.match(page, /className="match-time-control"[\s\S]{0,220}?Hora[\s\S]{0,220}?<input[\s\S]{0,220}?type="time"/);
