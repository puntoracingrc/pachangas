...
### R6B-ENVIRONMENT-111 - Activation orchestrator omits Web Crypto

- Classification: `ENVIRONMENT_ISSUE`
- Status: `FIXED / REGRESSION_VERIFIED`
- Original scenario: generate five idempotent operation IDs inside the tool
  orchestration isolate before the staged production flag RPCs.
- Observed: the isolate raises `ReferenceError: crypto is not defined` before
  constructing or sending the first SQL request.
- Impact: zero activation commands execute; production remains at revision 11
  with every R6B flag OFF.
- Planned correction: generate the five UUIDs with the local operating-system
  UUID utility, validate their shape, and replay the same audited RPC sequence
  from the unchanged expected revision.
- Regression evidence: five lower-case UUIDs generated by the operating-system
  utility were accepted in order by the audited platform RPC. Production moved
  monotonically from revision 11 to 16 and stored exactly one receipt/event for
  each activation operation.

### R6B-ENVIRONMENT-112 - Management SQL session is not service authority

- Classification: `ENVIRONMENT_ISSUE`
- Status: `FIXED / REGRESSION_VERIFIED`
- Original scenario: invoke the first audited Group Stage flag RPC through the
  Supabase Management SQL connection after generating valid operation IDs.
- Observed: the function reaches its authority guard and rejects the session
  with `42501 AUTHENTICATION_REQUIRED`; Management SQL does not impersonate the
  product `service_role`.
- Impact: no receipt or flag mutation is stored and production remains at
  revision 11 with R6B OFF.
- Planned correction: call the same platform RPC under a recognized platform
  role, preserving its idempotency/revision checks and never issuing a direct
  settings-table update.
- Regression evidence: each activation ran through
  `command_pachanga_tournament_group_stage_platform_v1` under the recognized
  transaction-local `service_role` claim. No direct settings update was used;
  readback confirms revision 16, sequences 1281-1285 and all five audited
  receipts. Replaying the fifth operation returned the identical receipt,
  retained revision/sequence and left one receipt plus one event.

### R6B-TEST-113 - Final readback assumes separate bracket source relations

- Classification: `TESTABILITY_GAP`
- Status: `FIXED / REGRESSION_VERIFIED`
- Original scenario: collect the post-activation production snapshot for flags,
  empty R6B aggregates, canonical matches and the five audited activation
  receipts before running the reversible canary.
- Observed: the diagnostic queries
  `pachanga_competition_bracket_templates`,
  `pachanga_competition_bracket_slots` and a separate bracket-source relation;
  R6B canonically stores templates and slots in
  `pachanga_tournament_bracket_templates` and
  `pachanga_tournament_bracket_slots`, with each source encoded on its slot.
  PostgreSQL rejects the statement with `42P01` before returning a snapshot.
- Impact: zero writes occur and no partial readback is accepted; activation and
  persisted product state are unchanged.
- Planned correction: bind the final readback to the exact migration-defined
  tournament bracket relations, derive source count from bracket slots and
  rerun the complete statement before the production canary.
- Regression evidence: the catalog-bound readback completed against
  `pachanga_tournament_bracket_templates` and
  `pachanga_tournament_bracket_slots`. It confirmed all R6B product relations
  empty before the canary, five activation receipts, revision 16 and every
  knockout/progression/public-discovery gate OFF.

### R6B-TEST-114 - Production canary expected revision is ambiguous

- Classification: `TESTABILITY_GAP`
- Status: `FIXED / REGRESSION_VERIFIED`
- Original scenario: execute the reversible four-team production canary through
  the real Draw, Group Stage, R4B publication and Tournament Hub paths.
- Observed: the expected-revision subquery joins the Group Stage state to its
  Competition but selects the unqualified column name `revision`; both
  relations expose that name and PostgreSQL aborts with `42702` before the
  command can continue.
- Impact: the surrounding transaction is aborted and rolled back. No QA user,
  team, grant, Tournament, schedule, CanonicalMatch, context, result or
  standings row is persisted.
- Planned correction: select `states.revision` explicitly at every R6B canary
  command boundary, verify the rollback baseline, then rerun the exact same
  four-team scenario.
- Regression evidence: every command now selects `states.revision` explicitly.
  The corrected canary crossed prepare, slot creation, generation, validation
  and publication without an ambiguous reference and its final rollback
  readback remained clean.

### R6B-SIMULATION-115 - Reused R6A fixture resets the narrow R6B gate

- Classification: `SIMULATION_BUG`
- Status: `FIXED / REGRESSION_VERIFIED`
- Original scenario: rerun the corrected four-team production canary by
  adapting the already certified R6A draw fixture before invoking R6B schedule
  generation.
- Observed: the inherited setup repeats its old Foundation/Tournament flag
  activation inside the transaction. The hardened compatibility path correctly
  leaves narrow Group Match Generation disabled for that in-transaction state,
  so R6B rejects generation with
  `TOURNAMENT_GROUP_MATCH_GENERATION_DISABLED`.
- Impact: the transaction is aborted and rolled back; the real production
  revision 16 and its active R6B flags remain unchanged, with zero QA rows.
- Planned correction: reuse only the R6A entity/draw/grant fixture, omit its
  obsolete flag-reset calls, and rely on the independently read-back production
  activation before executing the real R6B commands.
- Regression evidence: the canary retained only the temporary organizer grant
  and R6A draw entities, omitted the legacy flag-reset commands, consumed the
  independently verified production revision 16 and published six canonical
  Group matches while Group Match Generation remained ON.

### R6B-TEST-116 - Empty standings assertion counts group projections

- Classification: `TESTABILITY_GAP`
- Status: `FIXED / REGRESSION_VERIFIED`
- Original scenario: validate the required empty standings after the reversible
  canary has published six CanonicalMatches without recording any result.
- Observed: the Hub correctly returns one standings projection for the one
  Group, with no persisted StandingState and no played/result rows. The canary
  incorrectly interprets `standings.length = 1` as non-empty standings instead
  of checking that the projection's `rows` array is empty.
- Impact: the sporting flow itself reaches 4 teams, 1 Group, 1 leg, 3 rounds,
  6 fixtures, 6 CanonicalMatches, 6 MatchContexts and 0 results, then rolls
  back. Only the diagnostic verdict is `FAIL`.
- Planned correction: require exactly one Group standings projection and zero
  rows inside it, while retaining the database assertions for zero
  StandingStates and zero SportingResults.
- Regression evidence: the final canary proves one Group standings projection
  with zero rows, zero persisted StandingStates and zero SportingResults. Its
  exact facts are 4 teams, 1 Group, 1 leg, 3 rounds, 6 fixtures, 6
  CanonicalMatches and 6 MatchContexts, followed by complete rollback.

### R6B-ENVIRONMENT-117 - PWA probe cannot resolve the global navigator

- Classification: `ENVIRONMENT_ISSUE`
- Status: `FIXED / REGRESSION_VERIFIED`
- Original scenario: inspect the public production tab for installed display
  mode, Service Worker registrations, active controller and online state after
  Demo World V2.5 loads without console errors.
- Observed: the first read-only browser-evaluation closure resolves the bare
  `navigator` binding as undefined and stops before collecting PWA evidence.
- Impact: no browser or product state changes. The page, Demo V2.5 and console
  remain healthy, but PWA status is still unproven.
- Planned correction: use the browser's supported page/global or CDP
  inspection path, then verify registration, controlled reload and offline
  shell explicitly before closing the Service Worker gate.
- Regression evidence: CDP inspection of the controlled production target
  returned the active `/sw.js` registration, root scope, `activated` state and
  no waiting/installing worker without relying on the unavailable bare binding.

### R6B-ENVIRONMENT-118 - Persistent browser QA binding collides with matrix name

- Classification: `ENVIRONMENT_ISSUE`
- Status: `FIXED / REGRESSION_VERIFIED`
- Original scenario: traverse all ten Tournament tabs across the eight required
  desktop, portrait and landscape viewport overrides.
- Observed: the persistent browser session already contains an immutable
  identifier named `r6bTabs`; JavaScript rejects the matrix script at parse
  time with a duplicate-declaration error.
- Impact: the responsive loop never begins and no page/browser/product state is
  changed; no visual result is accepted from that attempt.
- Planned correction: use uniquely scoped or redeclarable bindings for the QA
  matrix, then rerun all 80 tab/viewport combinations from the start.
- Regression evidence: the rerun used uniquely named persistent bindings and
  completed all ten Tournament tabs at every required viewport.

### R6B-ENVIRONMENT-119 - Monolithic visual matrix exceeds browser dispatch deadline

- Classification: `ENVIRONMENT_ISSUE`
- Status: `FIXED / REGRESSION_VERIFIED`
- Original scenario: execute all 80 Tournament tab/viewport interactions in one
  browser-control request after resolving the persistent identifier collision.
- Observed: the browser deadline is reached before dispatching a click on the
  visible, enabled `Equipos` button. Locator diagnostics find exactly one valid
  target, so the interruption is orchestration duration rather than UI state.
- Impact: the partially accumulated matrix is discarded and no viewport is
  certified from this attempt. Product state is read-only and unchanged.
- Planned correction: reset the result set and run one bounded ten-tab batch per
  viewport, then consolidate only complete batches.
- Regression evidence: eight bounded viewport batches completed 80/80 tab and
  viewport combinations with no root overflow, clipped active control, broken
  image or pending image.

### R6B-TEST-120 - Demo tab detector assumes aria-pressed

- Classification: `TESTABILITY_GAP`
- Status: `FIXED / REGRESSION_VERIFIED`
- Original scenario: consolidate the bounded 80-combination responsive matrix
  after every tab click completes without root overflow or broken images.
- Observed: the diagnostic searches only for `aria-pressed="true"`, while the
  Demo navigation exposes its active state through a different existing
  attribute. It therefore reports every active tab as mismatched and cut even
  though all clicks completed.
- Impact: zero product writes and no visual PASS is accepted. The valid partial
  evidence remains zero root overflow, zero broken images and zero pending
  images across all completed combinations.
- Planned correction: inspect the exact rendered button attributes, bind the
  detector to the real active-state contract and rerun the complete viewport
  matrix.
- Regression evidence: the detector now reads the rendered
  `aria-current="page"` contract. All 80 combinations identify the selected tab
  and report zero active-control clipping.

### R6B-ENVIRONMENT-121 - CDP display-mode emulation does not enter standalone

- Classification: `ENVIRONMENT_ISSUE`
- Status: `FIXED / REGRESSION_VERIFIED`
- Original scenario: apply a 390x844 viewport plus CDP
  `display-mode: standalone` media emulation to the public production PWA.
- Observed: viewport emulation applies, but Chromium keeps
  `matchMedia('(display-mode: standalone)').matches = false`; the connected QA
  browser cannot represent an installed app window through that command.
- Impact: responsive portrait layout remains verifiable, but this attempt is
  not accepted as physical/installed standalone QA. Service Worker control and
  offline behavior can still be tested independently.
- Planned correction: certify the actual Service Worker controller and offline
  shell now, retain installed physical PWA as explicitly pending, and do not
  mislabel media emulation as a real installation.
- Regression evidence: installed display mode is no longer inferred from CDP
  media emulation. Service Worker and offline behavior are certified
  independently; physical installed-PWA QA remains explicitly `PENDING`, not a
  false PASS.

### R6B-ENVIRONMENT-122 - First offline reload is measured before lazy domain recovery

- Classification: `ENVIRONMENT_ISSUE`
- Status: `FIXED / REGRESSION_VERIFIED`
- Original scenario: set the connected browser offline, reload the already
  controlled Demo Tournament route, inspect the cached shell and immediately
  restore connectivity.
- Observed: the HTML title and Service Worker controller remain present with no
  navigation error, but the probe runs before the lazy Tournament domain has
  reappeared and the backend does not reflect the emulated state through
  `navigator.onLine`. The result cannot distinguish a cache miss from premature
  measurement.
- Impact: no product write and no offline verdict. Connectivity is restored and
  the production URL recovers normally.
- Planned correction: first wait for the online Tournament heading, prove the
  emulated network rejects an uncached request, then perform a longer offline
  reload and inspect the rendered fallback/domain state before restoring
  connectivity.
- Regression evidence: the corrected probe first waited for the online
  Tournament heading, then proved an uncached API request failed under CDP
  network blocking and waited for the complete offline reload result before
  restoring connectivity. That run exposed `R6B-PRODUCT-123` rather than
  reporting an ambiguous timing result.
...
- Original scenario: load Demo World V2.5 Tournament online until the canonical
  ...
  URL.
- Observed: the Service Worker serves the application shell, but the Demo data
  loader fails with `Failed to fetch` and renders `No se pudo abrir el Mundo
  Demo` instead of the previously visited canonical snapshot.
- Impact: installed clients cannot read the cached Demo Tournament/Hub offline,
  despite the permanent PWA contract allowing cached read models. No sporting
  write or fake success occurs.
- Planned correction: persist or cache the immutable Demo World snapshot using
...
- Classification: `PRODUCT_BUG`
- Status: `FIXED / REGRESSION_VERIFIED`
- Original scenario: execute the exact source returned by `buildServiceWorkerSource`
  inside the online-to-offline Demo chunk regression instead of inspecting it
  only as text.
- Observed: JavaScript rejects the generated worker with `SyntaxError: Invalid
  regular expression flags`; the template emits
  `/^/competiciones/[0-9a-f-]{36}/torneo$/i` because its slash escapes are
  consumed while constructing the source string.
- Impact: a newly downloaded worker cannot install. A previously active worker
  can keep controlling an existing browser and conceal the failed update, so
  clients may remain on an obsolete cache policy.
- Planned correction: escape the route-pattern slashes at the template-source
  boundary, execute the complete generated worker in the regression harness,
  and prove that an immutable Demo chunk warms online and is returned after the
  simulated network fails.
- Regression evidence: the generated worker now compiles in a VM harness. The
  same executable regression warms the hashed Tournament JSON online, disables
  its network implementation and reads the identical canonical payload from
  the worker cache; focused PWA/Demo tests pass 56/56.

### R6B-TEST-125 - PWA contract test protects the template escape, not the worker

- Classification: `TESTABILITY_GAP`
- Status: `FIXED / REGRESSION_VERIFIED`
- Original scenario: run the complete repository test suite after correcting
  the generated navigation regex and immutable Demo chunk cache policy.
- Observed: the R6B PWA test rejects the additional template-string escaping
  because it inspects `app/service-worker-source.ts` as text. Its expected
  fragment corresponds to the source layer that produced invalid JavaScript,
  while it never compiles the generated worker consumed by the browser.
- Impact: one false regression blocks the otherwise green suite and could
  encourage restoring the invalid worker expression.
- Planned correction: compile `buildServiceWorkerSource(...)` in the contract
  test and assert the generated route policy, so syntax validity at the browser
  boundary becomes the protected behavior.
- Regression evidence: the R6B contract test now builds and compiles the exact
