...
| GVC-020 | Google Places inputs | authenticated Preview | desktop/mobile | DEUDA_ESTRUCTURAL | P2 | The remote Google library warned that the active `google.maps.places.Autocomplete` widget is legacy and unavailable to new customers. | Registered in commit `466f3b3` before correction. `PlaceAutocompleteElement` is now primary, `(cities)` and Spain restrictions are preserved, and the legacy widget remains fallback-only. Source regression plus warning-free authenticated Preview at 390x844 and 844x390. Issue #166 then verified real predictions, keyboard and pointer selection, `gmp-select`, `fetchFields`, visible `gmp-error`, stale-selection invalidation, official field/match persistence, canonical reload, offline fail-closed and six responsive viewports against an isolated authenticated Preview. | FIXED_REGRESSION_VERIFIED |
