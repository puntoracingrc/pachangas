...
| DW2-012 | SIMULATION_BUG | fixed + regression_verified | El comparador congelado ignoraba `migrationCount`, pero no la clave numerica `migrations`; una migracion aditiva producia deriva falsa `204` frente a `212` aunque la autoridad deportiva fuese identica. El test de Wave 8A dependia ademas de la ubicacion privada del helper. | La proyeccion se aisla en un helper compartido que normaliza ambas claves antes de comparar valores; el test heredado valida ahora el contrato compartido. | Test funcional compara objetos 204/212 y `demo-world:v2:verify` confirma V3.0 identica con ledger 212 sin regenerar su prueba deportiva. |
